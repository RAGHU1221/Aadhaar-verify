...
4. When it finishes (green ✅ or red ❌), open the run and scroll to
   **Artifacts** at the bottom — two will be there:
   - **"AadhaarDocVerifier-Setup"** — a proper installer
     (`AadhaarVerifier-Setup.exe`, built with Inno Setup, see
     `windows/installer/aadhaar_verifier.iss`). Download and double-click
     it on the target PC; it installs per-user (no admin/UAC prompt
     needed) with a Start Menu shortcut and uninstaller. **Use this one**
     unless you specifically want the raw folder.
   - **"AadhaarDocVerifier-windows"** — the raw build folder as a zip
     (portable, no installer), if you'd rather just unzip and run.

   Either way, Tesseract-OCR still needs installing separately on the
   target PC (same as always).
...
├── windows/installer/aadhaar_verifier.iss  # Inno Setup script -> installer .exe
